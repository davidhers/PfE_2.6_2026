...
These datasets are to big to show here, but the key variables will be explained now. In the first dataset, the health per region with 9 different values, only the region, lightly overweight and obesity columns are needed. In the rows not everything is important aswell, but that will be discussed in the data cleaning. The second dataset, the percentage of overweight people over the years, is divided into underweight, overweight and obesity. Only overweight and obesity will be used, and of course the years. This will be used for total percentage of people, total percentage of men and total percentage of women. The third dataset contains information about income. The only necessary variables in this dataset are the province and the average income per person for these province The last dataset is about education level for every province. This is divided into five different education levels that will be needed, and again the provinces. \# 3 Quantifying


# 3 Data cleaning and creating new variables
 

```{r data_cleaning_1, include = TRUE}
overgewicht_1981 = overgewicht_over_tijd[1,5]
overgewicht_2025 = overgewicht_over_tijd[13,5]
#Selecting data for making the variable for the percentage change over time 
```
...
## 3.1 Data cleaning final dataset one
...
These four different dataset needed to be merged into two dataset with only the necessary variables. Firstly, the overweight over time dataset was used. The data that was needed was the total percentage of total overweight and obesity, and the percentage of total overweight in men and women. Since these were not on the same rows, this was done by slicing and then full_joining them based on the year they were in. To this was a new variable introduced, the percentage point change compared to the last data point. Since this is impossible for the first year, NA got added there. This is to see how and how much the percentage of overweight people change, this gave the first final dataset looking like this (with two more columns).
...
From left to right the columns are; year, total percentage of overweight people, total percentage of obese people, total percentage of overweight men, total percentage of overweight women and percentage point change of the total percentage of overweight people.

## 3.2 Data cleaning final dataset two

```{r data_cleaning_4,include=FALSE}
opleiding_provincie <- 
...
```{r data_cleaning_5, include= FALSE}
...
#The code broke for once, where "Fryslân" caused a problem. When restarting the Rmd it did work.
```

```{r data_cleaning_6, include=FALSE}
...
```{r data_GGD_regions(AlleCijfers, 2026), include=FALSE}
...
Then the other dataset for different provinces was created. Firstly the dataset of education level was used. Two new variable got created, the total percentage of lower educated people, by adding two columns to eachother. Then the total percentage of higher educated people, also by adding two columns together. Both higher and lower education were split into two different columns. For middle level of education this was not the case. Then we sliced out all the regions that werent needed, so only the provinces were left. The next thing for this dataset that was taking extracting the needed columns from the income dataset. This was the only thing for this that was done. These two datasets were then added together so that their province name could be set to the specific name needed later on. Another dataset was needed to be added to this one. This dataset contained the percentage of overweight people per region, but was not divided by province but by GGD regions. These regions needed to be converted to provinces. These regions are always only in one province, making this fairly easy. The following function got used for this:

```{r data_cleaning_function,include=TRUE}
...
The data that was needed for the populations of the regions was found on AlleCijfers (2026). Regio1, regio2, etc. were the amount of people living in the region, and data1, data2, etc was the percentage of overweight people. totaal_provincie was the total amount of people living in the provincie. Each factor was set to zero, because each province consists of a different amount of GGD regios, and like this no errors would happen.
...
After all this data was created, and put into one data.frame, this data got left_joined by province to create the final dataset for the maps, looking like this, with 3 more columns not fitting:
...
again, from left to right, including the last three columns that arent visible; Province, total percentage of overweight people, percentage of lower educated people, percentage of middle educated people, percentage of higher educated people, average income per person per province (x1000). These got then left_joined with the data for the map, to create one dataset to plot the 4 maps.
...
#One dataset, with the information for different provences gets merged with the provinces map of the Netherlands, to make 4 different maps to compare them.

```

```{r, fig.align='left', fig.width=7, fig.height=7, echo=FALSE}
combined_plot <-
  (ggplot(nl_map) +
...
#The dataset for the maps gets used for four different plots. These get combined into one big plot. These maps get created with geom_sf
```
...
Overweight and SES project:

-  README.md

-  overweight_in_NL_2,6.Rmd

-  data/
  - gewicht_regios.rds
  - opleiding_dataset.rds
  - inkomen_dataset.rds
  - overgewicht_over_tijd.rds
